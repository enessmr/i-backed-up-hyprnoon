...
const scaleSettings = new Gio.Settings({
...
const OsdValue = ({
  ...
  const valueName = MaterialIcon(`${name}`, "large");
  ...
  return Box({
    hexpand: true,
    className: `osd-bg  ${extraClassName}`,
    attribute: {
      disable: () => {
        valueNumber.label = "󰖭";
      },
...
const scaleIndicator = OsdValue({
  ...
  progressSetup: (self) => {
    ...
    const MAX_SCALE = 2.0;
    self.value = Math.min(
      Math.max((previousScale - MIN_SCALE) / (MAX_SCALE - MIN_SCALE), 0),
      1
    );
    ...
    child: Box({
      children: [
        ...
        Box({
          hpack: "fill",
          spacing: 15,
          children: [brightnessIndicator, volumeIndicator, scaleIndicator],
          setup: (self) =>
            self.hook(useCorners, () => {
              self.className = useCorners.value
                ? "osd-value osd-round"
                : "osd-value osd-full-round osd-elevation";
            }),
        }),
