...
export default (id) => {
  ...
  const widgetContent = Widget.Box({
    ...
    children: [CheatsheetHeader(), sheets],
  });
  return PopupWindow({
    monitor: id,
    name: `cheatsheet${id}`,
    ...
    setup: (self) =>
      self.on("key-press-event", (widget, event) => {
        ...
        if (
          checkKeybind(
            event,
            userOptions.asyncGet().keybinds.cheatsheet.nextTab
          )
        )
          ...
          checkKeybind(
            event,
            userOptions.asyncGet().keybinds.cheatsheet.prevTab
          )
        )
          ...
          checkKeybind(
            event,
            userOptions.asyncGet().keybinds.cheatsheet.cycleTab
          )
        )
          sheetContents.forEach((tab) => tab.cycleTab());
        // Keybinds
        if (
          sheets.attribute.names[sheets.attribute.shown.value] == "Keybinds"
        ) {
          // If Keybinds tab is focused
          if (
            checkKeybind(
              event,
              userOptions.asyncGet().keybinds.cheatsheet.keybinds.nextTab
            )
          ) {
            sheetContents.forEach((sheet) => {
              const toSwitchTab =
                sheet.attribute.children[sheet.attribute.shown.value];
              toSwitchTab.nextTab();
            });
          } else if (
            checkKeybind(
              event,
              userOptions.asyncGet().keybinds.cheatsheet.keybinds.prevTab
            )
          ) {
            sheetContents.forEach((sheet) => {
              const toSwitchTab =
