...
export const DesktopEntryButton = (app) => {
  ...
  const isFile =
    (app.iconName !== null && app.iconName.startsWith("~")) ||
    app.iconName.startsWith(".") ||
    app.iconName.startsWith("/");
  const css = `background-size:cover;background-image:${
    isFile ? `url('${expandTilde(app.iconName)}')` : "none"
  };`;
  return Widget.Button({
    className: "overview-search-result-btn",
    onClicked: () => {
...
export const SearchButton = ({ text = "" }) => {
  const search =
    searchConfig.engineBaseUrl +
    text +
    searchConfig.excludedSites.reduce(
      (acc, site) => (site ? acc + ` -site:${site}` : acc),
      ""
    );
...
export const AiButton = ({ text }) =>
  searchItem({
    ...
    content: text,
    onActivate: () => {
      (aiConfig.onSearch == "gemini" ? GeminiService : ChatGPTService).send(
        text
      );
      App.closeWindow("overview");
