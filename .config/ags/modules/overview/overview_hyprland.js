  ...
  const Window = (
    ...
    return Widget.Button({
      attribute: {
        ...
        updateIconSize: (self) => {
          appIcon.size =
            (Math.min(self.attribute.w, self.attribute.h) * scale) / 2.5;
        },
      },
              ...
              child: Widget.Revealer({
                ...
                child: Widget.Label({
                  ...
                  className: `margin-top-5 ${
                    xwayland ? "txt txt-italic" : "txt"
                  }`,
                  css: `
                                        font-size: ${
                                          (Math.min(
                                            monitors[monitor].width,
                                            monitors[monitor].height
                                          ) *
                                            scale) /
                                          14.6
                                        }px;
      ...
      attribute: {
        put: (widget, x, y) => {
          ...
          const newCss = `
                        ...
                        margin-right: -${Math.round(
                          x + widget.attribute.w * opts.overview.scale
                        )}px;
                        margin-bottom: -${Math.round(
                          y + widget.attribute.h * opts.overview.scale
                        )}px;
                    `;
          widget.css = newCss;
    ...
    const WorkspaceNumber = ({ index, ...rest }) =>
      Widget.Label({
        ...
        css: `
                margin: ${
                  Math.min(
                    monitors[overviewMonitor].width,
                    monitors[overviewMonitor].height
                  ) *
                  opts.overview.scale *
                  opts.overview.wsNumMarginScale
                }px;
                font-size: ${
                  monitors[overviewMonitor].height *
                  opts.overview.scale *
                  opts.overview.wsNumScale
                }px;
            `,
        setup: (self) =>
    ...
    const widget = Widget.Box({
      ...
      css: `
                min-width: ${
                  1 +
                  Math.round(
                    monitors[overviewMonitor].width * opts.overview.scale
                  )
                }px;
                min-height: ${
                  1 +
                  Math.round(
                    monitors[overviewMonitor].height * opts.overview.scale
                  )
                }px;
            `,
      children: [
            ...
            .then((clients) => {
              ...
              allClients.forEach((client) => {
                if (client.workspace.id === id) {
                  const screenCoords = box.attribute.monitorMap[client.monitor];
                  kids[id - (offset + startWorkspace)]?.set(
                    client,
                    screenCoords
                  );
                }
